...
public class MatriculacionesController {
	...
	public String Listamatriculas(@RequestParam(value="idAsig",required = false)Integer idAsignatura,@RequestParam(value="nombreAsig",required = false)String nombreAsignatura,@RequestParam(value="idAlum",required = false)Integer idAlumno,
			@RequestParam(value="nombreAlum")String nombreAlumno,@RequestParam(value="fecha")String fecha,ModelMap model) {
		model.addAttribute("lista",matriculas.obtenerMatriculas(idAsignatura, nombreAsignatura, nombreAlumno, idAlumno, fecha));
		return "vistas/matriculaciones/listadoMatriculaciones";
	}
	...
	public String InsertarPost(@RequestParam(value="asignaturas",required = false)Integer idAsignatura,@RequestParam(value="alumnos",required = false)Integer idAlumno,
			@RequestParam(value="tasa")Double tasa,@RequestParam(value="fecha")String fecha,ModelMap model) {
		model.addAttribute("listaAlumnos", combo.comboAlumnos());
		model.addAttribute("listaAsignaturas",combo.comboAsignaturas());
		matriculas.insertarMatriculas(idAsignatura, idAlumno, fecha,10.0);
	...
	public String FormularioBorrarMatriculasPost(@RequestParam(value="idAsig",required = false)Integer idAsignatura,@RequestParam(value="nombreAsig",required = false)String nombreAsignatura,@RequestParam(value="idAlum",required = false)Integer idAlumno,
			@RequestParam(value="nombreAlum")String nombreAlumno,@RequestParam(value="fecha")String fecha,ModelMap model) {
		model.addAttribute("lista",matriculas.obtenerMatriculas(idAsignatura, nombreAsignatura, nombreAlumno, idAlumno, fecha));
		return "vistas/matriculaciones/borrarMatriculaciones";
	}
	...
	@ResponseBody
	public Double calculaTasa(@RequestParam(value="idAsig",required = false)Integer idAsignatura,@RequestParam(value="idAlum",required = false)Integer idAlumno,
			@RequestParam(value="tasa")Double tasa,ModelMap model){
		model.addAttribute("listaAlumnos", combo.comboAlumnos());
		model.addAttribute("listaAsignaturas",combo.comboAsignaturas());
		tasa=nego.obtenerTasa(idAlumno, idAsignatura);
		return 25.0;
